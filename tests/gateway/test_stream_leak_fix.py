"""Regression tests for the quiet-channel queued-follow-up stream leak.

On 2026-09-18 the desk answered an operator's own posts inside a quiet
(counterparty-facing) Slack channel when a follow-up arrived mid-turn: the
queued-follow-up branch in gateway/run.py sent ``first_response`` directly to
``source.chat_id``, bypassing the ``_operator_unaddressed_in_quiet`` reroute
the completed-turn path applies.  These tests drive ``_run_agent`` with a
queued follow-up and assert the delivery target of the interrupted turn's
first response.
"""
