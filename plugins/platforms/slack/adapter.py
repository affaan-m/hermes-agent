...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _send_slash_ephemeral(
        ...
        Returns ``success=False`` on delivery failure. ``send()`` preserves
        that result without promoting a private reply into the public channel.
        The initial "Running /cmd…" ack stays up either way.
        """
    ...
    @staticmethod
    def _metadata_team_id(metadata) -> str:
        return str((metadata or {}).get("team_id") or (metadata or {}).get("team")
                   or (metadata or {}).get("slack_team_id") or "")
    ...
    async def send(
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                last_result = await self._get_client(chat_id, team_id=self._metadata_team_id(metadata)).chat_postMessage(**kwargs)
    ...
    async def send_private_notice(
        ...
        try:
            ...
            result = await self._get_client(chat_id, team_id=self._metadata_team_id(metadata)).chat_postEphemeral(**kwargs)
            return SendResult(
                success=True,
    ...
    async def edit_message(
        ...
        *,
        ...
        try:
            ...
            await self._get_client(chat_id, team_id=self._metadata_team_id(metadata)).chat_update(**update_kwargs)
            if finalize:
                await self.stop_typing(chat_id, metadata=metadata)
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        try:
            await self._get_client(chat_id, team_id=self._metadata_team_id(metadata)).assistant_threads_setStatus(
                channel_id=chat_id,
                thread_ts=thread_ts,
                status="is thinking...",
            )
    ...
    async def _upload_file(
        ...
        for attempt in range(3):
            try:
                result = await self._get_client(chat_id, team_id=self._metadata_team_id(metadata)).files_upload_v2(
                    channel=chat_id,
                    file=file_path,
        ...
        if is_mentioned:
            ...
            if mention_stripped.startswith("/"):
                ...
                try:
                    ...
                    pass
            # Retain thread context; participation still requires a new request.
