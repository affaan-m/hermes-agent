...
Backport note (2026-08-25, lane D): the canonical implementation merged on
affaan-m/hermes-agent#3 (0.20.x tree). This copy runs on the live 0.19.0
install: url_safety here has no SSRF-safe client factory, so the mint call
validates the endpoint with async_is_safe_url and disables redirect
following instead (no redirect-based SSRF; a redirect answers as
unavailable). Keep behavior in lockstep with the 0.20.x module.
"""
...
async def mint_link_nonce(
    *,
    ...
    if not await async_is_safe_url(mint_url):
        raise ItoLinkMintError("unconfigured", "mint URL failed the egress safety check")
